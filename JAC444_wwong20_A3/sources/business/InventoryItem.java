...
* Represents a real Inventory Item object
 ...
 @version 1.0
*/
public class InventoryItem implements Testable, Serializable
{
   //----static data
  ...
  //----------------------------------------------------------------------retrieveData()
  /**
    * <p>Only be called from within class Database.</p>  
    * <p>Searches for a particular InventoryItem object in the Hashtable using keyValue.</p>
	*
	* <p>If no matching item is found the returned array contains two entries, with values as follows:<br />
	* -	The number of records returned (as a String, always “0” in this case).<br />
	* -	“0 record(s) selected for: [inventoryId]”.</p>
	
	* <p>If a matching item is found the returned array contains three entries with values as follows:<br />
	* - The number of records returned (as a String, always “1” in this case).<br />
	* -	A tilde separated list of the field names for fields whose data is returned.<br />
	* -	A tilde separated list of the values of the fields from the object found.<br /></p>

   */
  public static String[] retrieveData(Hashtable table, String keyValue)
  {
	String[] returnval = new String[3];
	InventoryItem ii = new InventoryItem();
	
	// System.out.println("This is keyValue " + keyValue);
	//System.out.println("This is in the hashtable " + table.toString());
	
	if (table.containsKey(keyValue))
	{
		ii = (InventoryItem) table.get(keyValue); 
		
		DateFormat dateFormat = new SimpleDateFormat("yyyy-MM-dd");
		String currentDate = dateFormat.format(ii.lastUpdated);
		
		//manually write the string tilde lists
		returnval[0] = "1";
		returnval[1] = "~inventoryId                         : " +
					   "~description                         : " +
					   "~pack                                : " +
					   "~quantityInStock                     : " +
					   "~unitPrice                           : " +
					   ...
					   "~totalSalesOrders                    : " +
					   "~lastUpdated                         : ~";
		returnval[2] = "~" + ii.inventoryId + 
					   "~" + (ii.description).substring(1, (ii.description).length() - 1) + //trim off the quotes in the beginning and end
					   "~" + (ii.pack).substring(1, (ii.pack).length() - 1) + 
					   "~" + ii.quantityInStock +
					   "~" + ii.unitPrice +
					   "~" + ii.reorderPoint +
					   "~" + ii.reorderQuantity +
	...
	{
		returnval[0] = "0";
		returnval[1] = "<p>0 record(s) selected for: " + keyValue + "</p>";
	}
